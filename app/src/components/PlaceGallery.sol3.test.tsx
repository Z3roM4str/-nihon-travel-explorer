...
describe("PlaceGallery SOL-3", () => {
  ...
  it("opens accessible fullscreen, exposes full credits, closes topmost, and restores its opener", async () => {
    ...
    const credits = within(lightbox).getByText("Créditos de la foto");
    credits.focus();
    fireEvent.keyDown(document, { key: "Tab" });
    expect(document.activeElement).toBe(within(lightbox).getByRole("button", { name: "Cerrar pantalla completa" }));
    fireEvent.click(credits);
    const sourceLink = within(lightbox).getByRole("link", { name: "Fuente de prueba 1" });
    const licenseLink = within(lightbox).getByRole("link", { name: "CC BY 4.0" });
    sourceLink.focus();
    fireEvent.keyDown(document, { key: "Tab" });
    ...
    expect(document.activeElement).toBe(licenseLink);
