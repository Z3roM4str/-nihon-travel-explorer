...
export function Discovery({ places, hubs, state, savedIds, onToggle, onState, onOpen, onRegions }: Props) {
  ...
  <optgroup label="Otras experiencias">
    ...
    {hasActiveFilters && <div className="astra-active-filters"><span>Vista filtrada{hub ? ` · ${hub}` : ""}{filters.query ? ` · “${filters.query}”` : ""}{activeCount ? ` · ${activeCount} filtros` : ""}</span><button type="button" onClick={() => onState({ ...state, hub:null, query:"", categories:[], grades:[], planningBlocks:[], hiddenGemStatuses:[], tourismLevels:[], reservation:"all", page:1 })}>Limpiar filtros</button></div>}
