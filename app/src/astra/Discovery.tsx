...
export function Discovery({ places, hubs, state, savedIds, onToggle, onState, onOpen, onRegions }: Props) {
  ...
  return <>
    ...
    <section className="astra-controls" aria-label="Buscar lugares">
      ...
      <div className="astra-quick"><label><span className="visually-hidden">Hub</span><select value={hub ?? ""} onChange={e => setHub(e.target.value)}><option value="">Todo Japón ({places.length})</option>{hubs.map(h => <option key={h} value={h}>{h} ({places.filter(place => place.hub === h).length})</option>)}<option value="__regions">Explorar por región</option></select></label><label><span className="visually-hidden">Experiencias</span><select value={filters.categories.length === 1 ? filters.categories[0] : ""} onChange={e => updateFilters({ ...filters, categories:e.target.value ? [e.target.value] : [] })}><option value="">Experiencias</option>{categories.map(c => <option key={c}>{c}</option>)}</select></label><button ref={filterButtonRef} type="button" className="astra-filter-button" onClick={() => setFiltersOpen(true)}>Filtros ({activeCount})</button></div>
    </section>
