...
type Props = { place: Place; saved: boolean; onToggle: (id: string) => void };

export function PlaceCard({ place, saved, onToggle }: Props) {
  ...
  return <article className="astra-card">
    <a className="astra-card__image-link" href={href} aria-label={`Ver detalles de ${place.name}`}>
      <div className="astra-card__frame">
        {image && !failed ? <img src={image.url} alt={image.alt} loading="lazy" decoding="async" onError={() => setFailed(true)} /> :
          <div className="astra-card__placeholder"><span aria-hidden="true">▧</span><span>{failed ? "No se pudo cargar la fotografía" : "Fotografía pendiente"}</span></div>}
      </div>
    </a>
    ...
    <div className="astra-card__body">
      ...
      <h2><a href={href}>{place.name}</a></h2>
