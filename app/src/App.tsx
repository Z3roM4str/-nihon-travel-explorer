...
export default function App() {
  ...
  const safeToggle = useCallback((id: string) => {
    const reviewer=review.store.activeReviewer;if(!reviewer){setPendingInterest(id);return;}
    const current=review.store.places[id]?.votes[reviewer]??"unreviewed";
    if(review.store.places[id]?.disposition==="discarded"&&current!=="yes"){if(!confirm("¿Volver a considerar este lugar?"))return;review.disposition(id,"candidate");}
    if(current==="yes")setRemovedVote({id,reviewer}); review.toggleInterest(id,reviewer);
  }, [review]);
  ...
  return <AppShell destination={destination} savedCount={reviewIds.length} reviewer={review.store.activeReviewer} onReviewer={review.chooseReviewer}>
    <div id="astra-content">
      ...
      {route.surface === "explore" && review.error && <PersistenceNotice error={review.error} onRetry={review.retrySave} />}
      ...
      {route.surface === "trip" && <section className="astra-trip">
        ...
        {review.error && (
          <PersistenceNotice error={review.error} onRetry={review.retrySave} />
        )}
        {review.legacyIds.length>0&&!review.store.legacyClaim&&review.store.activeReviewer&&<button type="button" onClick={()=>setClaimOpen(true)}>Estos guardados son míos ({review.legacyIds.length})</button>}
        {savedPlaces.length ? (
          <ul>
            {savedPlaces.map(p => {
