#!/usr/bin/env python3
"""Deterministic selector for Phase 4H A-grade photography batch II.

Pure/offline selection only. Implements the Phase 4G design contract exactly.
"""
import argparse
import json
...
DEFAULT_OUT = Path("data/visual/a-grade-photography-batch-ii.json")
...
def select_batch(places, photography_records, tranche_size=TRANCHE_SIZE):
    ...
    for place_id in photographed_ids:
        ...
        if place:
            cat = place["category"]
            photographed_by_category[cat] = photographed_by_category.get(cat, 0) + 1
    ...
    while len(selected) < tranche_size:
        ...
        for hub in hubs:
            ...
            def priority(place):
                cat = place["category"]
                return (
                    selected_by_category.get(cat, 0),
                    1 if is_temporal_risk(place) else 0,
                    eligible_by_category.get(cat, 0),
                    photographed_by_category.get(cat, 0),
                    utf16_key(place["id"]),
                )
