#!/usr/bin/env python3
"""Deterministic selector for Phase 4F A-grade photography batch I.

Pure/offline selection only. It never downloads photographs or changes photography
metadata. The selector implements the Phase 4E design contract exactly.
"""
import argparse
import json
...
TRANCHE_SIZE = 24
CARRIED_FAILED_CLOSED_IDS = {"JP-033", "JP-126", "JP-203", "JP-204"}
TEMPORAL_CATEGORIES = {"🎆 Eventos", "🐋 Fauna y experiencias estacionales"}
TEMPORAL_NAME_TOKENS = ("festival", "tournament", "animejapan")
DEFAULT_OUT = Path("data/visual/a-grade-photography-batch-i.json")
...
def select_batch(places, photography_records, tranche_size=TRANCHE_SIZE):
    ...
    if len(eligible) < tranche_size:
        raise ValueError("not enough eligible A-grade uncovered places")
    ...
    pools = {
        hub: [p for p in eligible if p["hub"] == hub]
        for hub in hubs
    }
...
def main():
    ...
    print(f"OK: wrote {manifest['trancheSize']} targets to {out}")
